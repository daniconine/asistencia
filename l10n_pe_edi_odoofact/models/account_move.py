...
class AccountMove(models.Model):
    ...
    l10n_pe_edi_origin_move_type = fields.Selection(
        selection=[("1", _("Invoice")), ("2", _("Ticket"))],
        string="Origin Move Type",
        compute="_compute_origin_move_data",
        readonly=True,
        states={"draft": [("readonly", False)]},
        store=True,
    )
    l10n_pe_edi_origin_move_serie = fields.Char(
        string="Origin Move Serie",
        compute="_compute_origin_move_data",
        readonly=True,
        states={"draft": [("readonly", False)]},
        store=True,
    )
    l10n_pe_edi_origin_move_number = fields.Integer(
        string="Origin Move Number",
        compute="_compute_origin_move_data",
        readonly=True,
        states={"draft": [("readonly", False)]},
        store=True,
    )
    l10n_pe_edi_origin_move_date = fields.Date(
        string="Origin Move Date",
        compute="_compute_origin_move_data",
        readonly=True,
        states={"draft": [("readonly", False)]},
        store=True,
    ...
    @api.depends("l10n_pe_edi_origin_move_id")
    def _compute_origin_move_data(self):
        for rec in self:
            rec.l10n_pe_edi_origin_move_type = (
                rec.l10n_pe_edi_origin_move_id
                and (rec.l10n_pe_edi_origin_move_id.name[0] == "F" and "1" or "2")
                or False
            )
            rec.l10n_pe_edi_origin_move_serie = (
                rec.l10n_pe_edi_origin_move_id
                and str(rec.l10n_pe_edi_origin_move_id.sequence_prefix)[0:4]
                or False
            )
            rec.l10n_pe_edi_origin_move_number = (
                rec.l10n_pe_edi_origin_move_id
                and rec.l10n_pe_edi_origin_move_id.sequence_number
                or 0
            )
            rec.l10n_pe_edi_origin_move_date = (
                rec.l10n_pe_edi_origin_move_id
                and rec.l10n_pe_edi_origin_move_id.invoice_date
                or False
            )
    ...
    def _get_document_values_generar_odoofact(self, ose_supplier):
        ...
        values = {
            ...
            "documento_que_se_modifica_tipo": self.l10n_pe_edi_origin_move_type
            and int(self.l10n_pe_edi_origin_move_type)
            or "",
            "documento_que_se_modifica_serie": self.l10n_pe_edi_origin_move_serie
            and self.l10n_pe_edi_origin_move_serie
            or "",
            "documento_que_se_modifica_numero": self.l10n_pe_edi_origin_move_number
            and self.l10n_pe_edi_origin_move_number != 0
            and self.l10n_pe_edi_origin_move_number
            or "",
    ...
    def action_l10n_pe_edi_move_cancel(self):
        self.l10n_pe_edi_request_id.action_api_connect("consultar")
        if self.l10n_pe_edi_request_id.sunat_canceled:
            self.l10n_pe_edi_sunat_canceled_progress = False
            self.button_draft()
            self.button_cancel()
        if self.state == "posted":
            return self.env['ir.actions.act_window']._for_xml_id('l10n_pe_edi_odoofact.l10n_pe_edi_move_cancel_action')
